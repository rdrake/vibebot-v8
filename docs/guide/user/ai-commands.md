...
memegen names most templates by the quote, so the bot also matches on the words you share with a name: `willy wonka` finds Condescending Wonka, `elmo fire` finds Elmo. When two templates tie it lists both rather than guessing. Every template also carries topic tags — the characters people name it by and the situations it fits — so `gym` reaches Butthurt Dweller and `monday` reaches the Will Smith slap; a topic shared by several templates (`cat`, `star wars`) lists them instead. Your operator can add names memegen lacks — Tyrone Biggums is filed under the quote, `yallgot`, and `tyrone` is aliased to it out of the box.
