...
class TestDrawContext:
    ...
    def test_build_draw_context_with_nick_and_channel(self) -> None:
        """GIVEN msg with nick and channel WHEN building draw context THEN includes both."""
        mock_irc = Mock()
        mock_irc.state = Mock()
        mock_irc.state.channels = {"#linux": Mock(topic="All things Linux")}
        mock_msg = Mock()
        mock_msg.prefix = "DragonSlayer99!user@host"
        mock_msg.args = ("#linux", "draw a penguin")

        result = self.service._build_draw_context(mock_irc, mock_msg)

        assert "Requested by: DragonSlayer99" in result
        assert "Channel: #linux" in result
        assert "Topic: All things Linux" in result

    def test_build_draw_context_without_topic(self) -> None:
        """GIVEN channel with no topic WHEN building draw context THEN omits topic."""
        mock_irc = Mock()
        mock_irc.state = Mock()
        mock_irc.state.channels = {"#test": Mock(topic="")}
        mock_msg = Mock()
        mock_msg.prefix = "alice!user@host"
        mock_msg.args = ("#test", "draw a cat")

        result = self.service._build_draw_context(mock_irc, mock_msg)

        assert "Requested by: alice" in result
        assert "Channel: #test" in result
        assert "Topic" not in result

    def test_build_draw_context_private_message(self) -> None:
        """GIVEN private message (no channel) WHEN building draw context THEN nick only."""
        mock_irc = Mock()
        mock_msg = Mock()
        mock_msg.prefix = "alice!user@host"
        mock_msg.args = ("botname", "draw a cat")
    ...
    def test_build_draw_context_none_msg(self) -> None:
        """GIVEN msg is None WHEN building draw context THEN returns empty string."""
        result = self.service._build_draw_context(Mock(), None)
        assert result == ""

    def test_image_generation_with_caller_context(self) -> None:
        """GIVEN irc and msg WHEN generating image THEN caller context prepended to prompt."""
        prompt_used = []
        ...
        with patch("llm.service.litellm.image_generation", side_effect=capture_prompt):
            self.service.image_generation("a penguin", irc=mock_irc, msg=mock_msg)

        assert len(prompt_used) == 1
        assert "[Requested by: DragonSlayer99" in prompt_used[0]
        assert "Channel: #linux" in prompt_used[0]
        assert "a penguin" in prompt_used[0]
